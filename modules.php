  ...
  <body style="background-color: black; overflow-x: hidden">
      ...
      <div class="col-4 text-center">
        ...
        <div class="btn-group mr-3">
          ...
          <ul class="dropdown-menu">
            <li>
              <a class="dropdown-item" href="modules.php?sort=htol" name="sorthtol"
                >Price (Hight to Low)</a
              >
            </li>
            <li>
              <a class="dropdown-item" href="modules.php?sort=ltoh" name="sortltoh"
                >Price (Low to High)</a
              >
            </li>
            <li>
            ...
            <li><a class="dropdown-item" href="modules.php">Clears sort</a></li
          </ul>
        </div>
    ...
    <div class="row ml-1 mr-1">
      <?php 
        ...
        $query="SELECT * FROM product WHERE category='Modules' ORDER BY price DESC;";
    ...
    <?php if($_GET['sort'] === "ltoh") {?>
      <div class="row ml-1 mr-1">
        <?php 
          require_once ('includes/conn.inc.php');
          $query="SELECT * FROM product WHERE category='Modules' ORDER BY price;";
          $result = mysqli_query($conn, $query);
          $resultCheck = mysqli_num_rows($result);
          if($resultCheck >0) { while($row = mysqli_fetch_assoc($result)) { ?>
        <div class="col-3 p-1">
    ...
    <?php if(!$_GET['sort']) {?>
      <div class="row ml-1 mr-1"">
        <?php 
          require_once ('includes/conn.inc.php');
          $query="SELECT * FROM product WHERE category='Modules';";
          $result = mysqli_query($conn, $query);
          $resultCheck = mysqli_num_rows($result);
          if($resultCheck >0) { while($row = mysqli_fetch_assoc($result)) { ?>
        <div class="col-3 p-1">
